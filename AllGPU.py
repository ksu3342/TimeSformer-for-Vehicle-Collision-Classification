...
class VideoDataset(Dataset):
    def __init__(self, root_dir, frame_size=(256, 256), num_frames=100):
        """
        初始化 VideoDataset 类的实例。

        :param root_dir: 包含视频文件的根目录路径。
        :param frame_size: 视频帧的目标尺寸，默认为 (256, 256)。
        :param num_frames: 每个视频提取的帧数，默认为 100。
        """
        # 保存根目录路径
    ...
    def __len__(self):
        ...
        :return: 视频样本的数量
        """
        # 返回 video_files 列表的长度，即数据集中视频样本的数量
